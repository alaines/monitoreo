 ...
 */
    ...
    public function consulta($data = null){
        ...
        if($data != null){
            ...
            if($data['fecha1'] <> '' AND $data['fecha2'] == ''){
                ...
                $conditions['Ticket.created >'] = $fecha1->format('Y-m-d H:i:s');
            }
            if($data['fecha1'] <> '' AND $data['fecha2'] <> ''){
                $fecha1 = new DateTime($data['fecha1']);
                $fecha2 = new DateTime($data['fecha2']);
                $conditions['Ticket.created BETWEEN ? AND ?'] = array($fecha1->format('Y-m-d H:i:s'), $fecha2->format('Y-m-d H:i:s'));
            }
        }
        $this->contain(array('Equipo'=>array('fields' => array('id','nombre')), 'Estado', 'Cruce' => array('Proyecto'), 'Prioridade','Incidencia'=>array('ParentIncidencia'), 'Reportadore'));
    ...
    public function reporte_estado($tiempo){
        /**
        $query = "SELECT 
                    SUM(CASE WHEN (t.estado_id = 1 OR t.estado_id = 5) THEN 1 ELSE 0 END) AS pendientes,
                    SUM(CASE WHEN (t.estado_id = 2) THEN 1 ELSE 0 END) AS proceso,
                    SUM(CASE WHEN (t.estado_id = 4) THEN 1 ELSE 0 END) AS atendidas
                  FROM 
                    public.tickets as t;";
        **/
        $condicion = 'WHERE t.created::date = current_date;';
        if($tiempo == 'ant'){$condicion = 'WHERE t.created::date < current_date;';}
        $query = "SELECT 
                    SUM(CASE WHEN (t.estado_id = 1 OR t.estado_id = 2 OR t.estado_id = 5) THEN 1 ELSE 0 END) AS pendientes,
                    SUM(CASE WHEN (t.estado_id = 4) THEN 1 ELSE 0 END) AS atendidas
                  FROM 
                    public.tickets as t ".$condicion;
